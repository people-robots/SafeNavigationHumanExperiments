...
## Predicts next position distribution using a kalman filter
#
class KFprediction:
	...
	def predict(self, observations, dt):
		action = np.zeros(observations[0].shape);
		for i in range(len(observations)):
			action += observations[i]*self.alpha[i]
	...
	def update(self):
		if self._robot.stepNum == 1:
			pos = [self._robot.location[0],self._robot.location[1]];
			pos.append(0)
			pos.append(0)
			self.pred_position = np.reshape(pos,(4,1))
		z = np.reshape(self._robot.location,(2,1)) + np.random.rand(2,1)*0.02 - 0.01; ##adding noise
	...
	def alphas(self,forces):
		...
		for i,f in enumerate(forces):
			...
			if self._alg == 'ours':
				self.alpha[i] = self.alpha[i] + self._lr*np.inner(f_new.flatten(),diff);
			else:
...
class Robot:
	...
	def NextStep(self, env):
		...
		input_control = self._nav_algo.select_next_action();
		try:
			control_input,dtime = input_control
		except:
		...
		if 'kf' in self._sensors:
			#dt = -self.last_time + dtime
			#if dt>100:
			#	dt = 6
			#self.last_time = dtime
			#print (dt)
			self._sensors['kf'].update();
			sfm = self._sensors['sfm'];
			sfm.select_next_action()
			observations = [sfm.target_vect, sfm.human_vect,
							sfm.obs_vect];
			self._sensors['kf'].alphas(observations);
			self._sensors['kf'].predict(observations,1/25);#25 frames per second
			self._pred_points.append(self._sensors['kf'].pred_position[0:2,0])
			#print (self._pred_points[-1].shape)
			self._pred_var.append(self._sensors['kf'].pred_distribution[0:2,0:2])
			self._SFM_error.append(self._sensors['kf']._error)
	...
	def draw(self, dtool):
		...
		if (0 < self._cmdargs.debug_level):
			...
			if 'kf' in self._sensors:
				dtool.set_color((255,255,0));
				dtool.set_stroke_width(0.05);
				self._draw_covariance(dtool,
									self._sensors['kf'].updated_position,
									self._sensors['kf'].updated_distribution[0:2,0:2]);
				dtool.set_stroke_width(0.1);
				dtool.draw_circle(self._sensors['kf'].updated_position.flatten(),0.05);
				dtool.set_color((255,0,255));
				dtool.set_stroke_width(0.05);
				self._draw_covariance(dtool,
									self._sensors['kf'].pred_position,
									self._sensors['kf'].pred_distribution[0:2,0:2]);
				dtool.set_stroke_width(0.1);
				dtool.draw_circle(self._sensors['kf'].pred_position.flatten(),0.05);
	...
	def _draw_covariance(self, dtool, center, cov_matrix):
		lambda_, v = np.linalg.eig(cov_matrix);
		order = lambda_.argsort()[::-1];
		v = v[:,order];
		lambda_ = np.sqrt(lambda_);
		dtool.draw_ellipse(center.flatten(),lambda_[0]*2,lambda_[1]*2,np.arctan2(*v[:,0][::-1]));
