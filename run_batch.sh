...
[[ -z "$base_args" ]] && base_args="--enable-memory --debug-level 1 --robot-speed 6 --radar-resolution 5 --batch-mode --max-steps=500 --robot-movement-momentum=0.0 --robot-memory-sigma=35 --robot-memory-decay=1 --robot-memory-size=500 --display-every-frame"
[[ -z "$num_trials" ]] && num_trials=1
