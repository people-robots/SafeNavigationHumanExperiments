...
def build_train_data(angle_tol = 0):
	...
	#data = {k: v for k, v in data.items() if k not in bad_peds}
	...
	for i in range(1):
		...
		shifter = 0 if i == 0  else random.randint(0,360)
...
def build_complete_train_data(angle_tol = 0):
	data_new        = {}
	actions         = {}
	cosined_actions = {}
	targets         = {}
...
def main():
	if network == 'action_m':
		...
		stds = [0, 5, 10, 15, 20, 25]
		distances = [0.35, 0.35, 0.35, 0.35, 0.35, 0.35]
		#stds = [5]
		#distances = [0.35]
		for std,dist in zip(stds,distances):
			print('Building train data (may take a while)...')
			data_new,targets,actions,cosined_actions,vel = build_complete_train_data(std)
			#data_new,targets,actions,cosined_actions,vel = build_train_data(std)
			print('Done building train data.')

			train('dnns/action_'+str(std)+'_'+str(dist)+'.dnn', data_new,targets,actions,cosined_actions,vel, dist)

			print('Building test data (may take a while)...')
			data_new,targets,actions,vel = build_complete_test_data()
			#data_new,targets,actions,vel = build_test_data()
			print('Done building test data.')

			cl_error,an_error,v_error = test('dnns/action_'+str(std)+'_'+str(dist)+'.dnn',data_new,targets,actions,vel)
		...
		for std,dist in zip(stds,distances):
			if dist != 0.35:
				data_new,targets,actions,cosined_actions,vel = build_complete_train_data(std)
				train('dnns/GRP_'+str(std)+'_'+str(dist)+'.dnn', data_new,targets,actions,cosined_actions,vel,dist)
			data_new,targets,actions,vel = build_complete_test_data()
			cl_error,an_error,v_error = test('dnns/GRP_'+str(std)+'_'+str(dist)+'.dnn',data_new,targets,actions,vel)
			if os.path.isfile('recording.json'):
				with open ('recording.json') as recording_data:
					recording = json.load(recording_data)
			else:
		...
		raise ValueError("Error: Unknown network: {}".format(network))
		sys.exit(1)
